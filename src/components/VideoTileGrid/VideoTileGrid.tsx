...
export const VideoTileGrid: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  const { tileIds: contentShareTileIds = [] } = useContentShareState() as any;
  const { isVideoEnabled } = useLocalVideo();

  // Use tileIds if tiles is not available
  const contentShareTiles = contentShareTileIds;
  ...
  return (
    <VideoGrid {...rest} size={gridSize} layout={featured ? 'featured' : null}>
      {contentShareTiles.length === 1 ? (
        ...
        <FeaturedArea>
          <ContentShareGrid>
            {contentShareTiles.map((tileId: number, index: number) => (
              <ContentShare key={tileId || index} />
            ))}
          </ContentShareGrid>
        </FeaturedArea>
      ) : null}
